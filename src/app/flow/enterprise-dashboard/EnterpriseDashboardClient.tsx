...
function getCircuitHealth(distribution: Record<Role, number>, total: number) {
  ...
  for (const role of roles) {
    ...
    if (pcts[role] === 0) {
      ...
      gaps.push(
        `${role} underrepresented (${pcts[role]}% vs. ideal ${roleConfig[role].ideal}%).`,
      );
    }
    if (
      pcts[role] >= roleConfig[role].ideal * 0.8 &&
      pcts[role] <= roleConfig[role].ideal * 1.3
    ) {
...
export default function EnterpriseDashboard() {
  ...
  const realTeams = useMemo<TeamSummary[]>(
    () => (teamsQuery.data ?? []) as TeamSummary[],
    [teamsQuery.data],
  );

  const allTeamNames = useMemo(() => {
    const names = realTeams.map((team) => team.name);
    return [...names, ...Object.keys(sampleTeams)];
  }, [realTeams]);
  ...
  const team: TeamMember[] = useMemo(() => {
    ...
    return ((teamAssessmentsQuery.data ?? []) as TeamAssessment[]).map(
      (assessment) => ({
        name:
          assessment.guestName ||
          assessment.guestEmail ||
          `Member #${assessment.id}`,
        role: assessment.role as Role,
  ...
  useEffect(() => {
    if (selectedTeam || allTeamNames.length === 0) return;
    const first = allTeamNames[0];
    const frame = window.requestAnimationFrame(() => {
      setSelectedTeam(first);
      setSelectedTeamId(
        first.startsWith("Demo:")
          ? null
          : (realTeams.find((team) => team.name === first)?.id ?? null),
      );
    });
    return () => window.cancelAnimationFrame(frame);
  }, [allTeamNames, realTeams, selectedTeam]);
  ...
  const health = useMemo(
    () => getCircuitHealth(distribution, team.length),
    [distribution, team.length],
  );
  ...
  const handleTeamSelect = (name: string) => {
    ...
    if (name.startsWith("Demo:")) {
      ...
      const realTeam = realTeams.find((team) => team.name === name);
    ...
    <div className="min-h-screen bg-background text-foreground pt-20">
      ...
      <section className="py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            See your team&#39;s entire circuit composition at a glance. Where
            are the gaps? Where&#39;s the overload? What does the relay actually
      ...
      <section className="pb-20 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          {selectedTeamId &&
            !teamAssessmentsQuery.isLoading &&
            team.length === 0 && (
              <div className="text-center py-12 bg-card/30 rounded-2xl border border-border">
                ...
                <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                  This team hasn&#39;t completed any Flow Circuit assessments
                  yet. Share the team code to get started.
                </p>
            ...
            <>
                  ...
                  <div className="relative w-36 h-36 mx-auto mb-4">
                    ...
                    >
                      ...
                      <circle
                        ...
                        strokeDasharray={`${health.score * 3.14} 314`}
                        strokeLinecap="round"
                      />
                    </svg>
                ...
                <div className="bg-card/50 backdrop-blur-sm rounded-2xl border border-border p-6">
                  ...
                  {health.strengths.length === 0 ? (
                    <p className="text-sm text-muted-foreground">
                      Rebalancing needed across all roles.
                    </p>
                  ) : (
                    <ul className="space-y-2">
              ...
              <div className="bg-card/50 backdrop-blur-sm rounded-2xl border border-border p-8 mb-10">
                ...
                <p className="text-xs text-muted-foreground mt-4">
                  Dashed lines indicate ideal distribution targets.
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {team.map((member, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-10 h-10 rounded-full ${cfg.bgColor}/20 flex items-center justify-center text-lg`}
                        >
                          {cfg.emoji}
                        </div>
              ...
              <div className="bg-gradient-to-br from-blue-500/5 to-cyan-500/5 rounded-2xl border border-blue-500/20 p-8 mb-10">
                ...
                {health.gaps.length > 0 ? (
                  <div className="space-y-4">
                    <p className="text-muted-foreground">
                      Your circuit has {health.gaps.length} gap
                      {health.gaps.length > 1 ? "s" : ""} that could be limiting
                      team performance. Consider these actions:
                    </p>
                    <div className="grid md:grid-cols-2 gap-4">
                      {distribution.Spark === 0 && (
                        <div className="p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/20">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Without a Spark, innovation stalls. Look for the
                            person who sees what doesn&#39;t exist yet.
                          </p>
                        </div>
                      )}
                      {distribution.Ground === 0 && (
                        <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Without a Ground, nothing ships. Find the person who
                            turns vision into deliverables.
                          </p>
                        </div>
                      )}
                      {distribution.Conductor === 0 && (
                        <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/20">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Without a Conductor, the relay has no sequencer.
                            Someone needs to read the room and route the energy.
                          </p>
                        </div>
                      )}
                      {distribution.Filter === 0 && (
                        <div className="p-4 rounded-xl bg-rose-500/5 border border-rose-500/20">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Without a Filter, quality drops. You need the person
                            who stress-tests every idea before it ships.
                          </p>
                        </div>
                      )}
                      {distribution.Amplifier === 0 && (
                        <div className="p-4 rounded-xl bg-cyan-500/5 border border-cyan-500/20">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Without an Amplifier, great ideas die quietly. Find
                            the person who can broadcast the signal.
                          </p>
                        </div>
                      )}
                      <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/20">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Have all team members take the assessment to get
                          precise circuit mapping.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <p className="text-green-400">
                    Your circuit is well-balanced. Focus on maintaining role
                    clarity and preventing drift.
                  </p>
                )}
              </div>
            </>
