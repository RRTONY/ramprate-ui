...
export default function DeepCalibrationClient() {
  ...
  const moveStatement = useCallback(
    (statementId: string, direction: "up" | "down") => {
      setCurrentOrder((order) => {
        const currentIndex = order.indexOf(statementId);
  ...
  const reorderStatement = useCallback((sourceId: string, targetId: string) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {phase === "intro" && (
        <div
          key="intro"
          className="flow-deep-calibration-phase-enter min-h-screen flex items-center justify-center px-4"
        >
          <div className="max-w-2xl mx-auto text-center space-y-8">
            <div className="flow-deep-calibration-badge-enter inline-flex items-center gap-2 px-4 py-2 rounded-full bg-violet-500/10 border border-violet-500/30 text-violet-400 text-sm font-medium">
            ...
            <p className="text-lg text-muted-foreground leading-relaxed max-w-xl mx-auto">
              The standard assessment tells you who you are. This tells you who
              you <em>can&#39;t pretend not to be</em>.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
              ...
              <Card className="bg-card/50 border-border/50">
                <CardContent className="p-5 space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Completing calibration earns a &quot;Calibrated&quot; badge
                    on your profile - it means you&#39;ve done the more rigorous
      ...
      {phase === "calibrating" && currentSetData && (
        <div
          key={`set-${currentSet}`}
          className="flow-deep-calibration-phase-enter flow-deep-calibration-phase-enter--side min-h-screen flex flex-col items-center justify-center px-4 py-12"
        >
          {/* Progress bar */}
          <div className="w-full max-w-xl mb-8">
            ...
            >
              <div
                className="flow-deep-calibration-progress-bar h-full bg-violet-500 rounded-full"
          ...
          <div className="max-w-xl w-full mb-6">
            ...
            <p className="text-sm text-muted-foreground text-center">
              ...
              to{" "}
              <strong className="text-muted-foreground">least like you</strong>{" "}
              (bottom)
            </p>
          </div>
              ...
              {currentOrder.map((stmtId, index) => {
                ...
                return (
                  <div
                    ...
                    draggable
                    onDragStart={(event) => {
                      setDraggedStatementId(stmtId);
                      event.dataTransfer.effectAllowed = "move";
                      event.dataTransfer.setData("text/plain", stmtId);
                    }}
                    ...
                    onDrop={(event) => {
                      ...
                      const sourceId =
                        ...
                        draggedStatementId;
                      if (sourceId) reorderStatement(sourceId, stmtId);
                      setDraggedStatementId(null);
                    }}
                    onDragEnd={() => setDraggedStatementId(null)}
                    className={`flow-deep-calibration-rank-item flex items-center gap-3 p-4 rounded-lg border cursor-grab active:cursor-grabbing transition-colors ${rankColors[index]}`}
                    data-dragging={draggedStatementId === stmtId || undefined}
                  >
                    ...
                    <div className="flex flex-col gap-1 shrink-0">
                      <button
                        type="button"
                        onClick={() => moveStatement(stmtId, "up")}
                        disabled={index === 0}
                        aria-label={`Move ${rankLabels[index]} statement up`}
                        className="flow-deep-calibration-rank-button"
                      >
                        <ArrowUp className="w-3.5 h-3.5" aria-hidden="true" />
                      </button>
                      <button
                        type="button"
                        onClick={() => moveStatement(stmtId, "down")}
                        disabled={index === currentOrder.length - 1}
                        aria-label={`Move ${rankLabels[index]} statement down`}
                        className="flow-deep-calibration-rank-button"
                      >
      ...
      {phase === "submitting" && (
        <div
          key="submitting"
          className="flow-deep-calibration-phase-enter min-h-screen flex items-center justify-center"
        >
          <div className="text-center space-y-4">
            <Target className="flow-deep-calibration-spinner w-12 h-12 text-violet-400 mx-auto" />
      ...
      {phase === "results" && results && (
        <div
          key="results"
          className="flow-deep-calibration-phase-enter min-h-screen py-16 px-4"
        >
          <div className="max-w-3xl mx-auto space-y-8">
            {/* Header */}
            <div className="text-center space-y-4">
              <div className="flow-deep-calibration-badge-enter inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm font-medium">
                    ...
                    return (
                      <div key={role} className="space-y-1">
                        <div className="flex justify-between text-sm">
                          <span className={`font-medium ${ROLE_COLORS[role]}`}>
                            {role}
                          </span>
                        ...
                        <div className="relative h-3 bg-muted rounded-full overflow-hidden">
                          ...
                          {/* Calibrated score */}
                          <div
                            className={`flow-deep-calibration-score-bar absolute inset-y-0 left-0 rounded-full ${
                              role === "Spark"
                                ? "bg-amber-500"
                                : role === "Amplifier"
                                  ? "bg-cyan-500"
