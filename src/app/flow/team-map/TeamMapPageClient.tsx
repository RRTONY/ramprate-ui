...
import { Users, ArrowRight, CheckCircle2, Copy } from "lucide-react";
import {
  Role,
  calculateRoleScores,
...
const MISSING_ROLE_IMPACT: Record<Role, string> = {
  Spark:
    "Without a Spark, the team stagnates - no one is generating the raw ideas that everything else builds on.",
  Amplifier:
    "Without an Amplifier, great ideas die in silence - no one is turning vision into organizational buy-in.",
  Filter:
    "Without a Filter, the team ships broken products and makes avoidable mistakes - no one is stress-testing the plan.",
  Ground:
    "Without Ground energy, nothing ships - ideas, momentum, and analysis stay theoretical.",
  Conductor:
    "Without a Conductor, the relay breaks down - each role operates in isolation and handoffs fail.",
};
...
export default function TeamMapPage() {
  ...
  const { data: domainResults, isLoading: domainLoading } =
    trpc.assessment.byDomain.useQuery(
      { domain: domain! },
      { enabled: !!domain },
    );
  ...
  const roleFrictionCards = useMemo(() => {
    ...
    const cards: {
      ...
      nameA: string;
      nameB: string;
      guide?: ReturnType<typeof getInteraction>;
    }[] = [];
    for (let i = 0; i < scatterNodes.length; i++) {
      for (let j = i + 1; j < scatterNodes.length; j++) {
        ...
        const isFriction =
          aInsight?.frictionWith.includes(b.role) ||
          bInsight?.frictionWith.includes(a.role);
        ...
        cards.push({
          ...
          nameB: b.name,
          guide:
            getInteraction(a.role, b.role) || getInteraction(b.role, a.role),
        });
      }
    }
    return cards;
      ...
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
        <div className="max-w-lg text-center space-y-6">
          ...
          >
            ...
            <Button
              type="submit"
              className="bg-yellow-400 text-black hover:bg-yellow-300 font-bold h-12 px-6"
            >
          ...
          <p className="text-sm text-gray-500">
            Or{" "}
            <Link href="/flow/assessment" className="text-yellow-400 underline">
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-5xl mx-auto px-4 py-8 md:py-12 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-gray-400 mb-1">
              Tribe Energy Map
            </p>
          ...
          >
            ...
            >
              <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#C8362A" />
              </svg>
              SPARK
            </span>
            <span
            ...
            >
              <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#D4622A" />
              </svg>
              AMPLIFIER
            </span>
            <span
            ...
            >
              <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#4A7C9E" />
              </svg>
              FILTER
            </span>
            <span
            ...
            >
              <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#6B8F71" />
              </svg>
              GROUND
            </span>
            <span
            ...
            >
              <svg width="10" height="10">
                <circle cx="5" cy="5" r="5" fill="#7B68AE" />
              </svg>
              CONDUCTOR
            </span>
            <span
        ...
        >
          ...
          >
            <strong>Vertical axis:</strong> Innovation (top) vs. Execution
            (bottom). Where ideas are born vs. where they land.
          </p>
          <p
            style={{
              fontSize: "15px",
          ...
          >
            <strong>Horizontal axis:</strong> Analysis (left) vs. Momentum
            (right). How people process vs. how people move.
          </p>
          <p
            style={{
              fontSize: "15px",
          ...
          >
            <strong>Node size:</strong> Larger = higher purity. A big node means
            that person is deeply, consistently one role. A smaller node means
            they blend two roles - which is why they can operate in both worlds.
          </p>
          <p
            style={{
              fontSize: "15px",
          ...
          >
            <strong>Red dashed lines:</strong> Friction, not conflict. These
            pairs see the world through different physics. The fix is never
            changing who they are. It is changing how they hand off.
          </p>
        </div>
        ...
        {roleFrictionCards.length > 0 && (
          <section style={{ margin: "40px 0" }}>
            ...
            >
              ...
              collide without a protocol, it drains energy from both people and
              from the mission. Here is what to do about each friction pair.
            </p>
              ...
              {roleFrictionCards.map((card, i) => (
                ...
                >
                  ...
                  >
                    <span
                      style={{
                        background: ROLE_HEX[card.roleA],
                        color: "#F4F0E8",
                        fontSize: "11px",
                        fontFamily: "'Cabinet Grotesk',sans-serif",
                        fontWeight: 700,
                    ...
                    >
                      {card.roleA.toUpperCase()} x {card.roleB.toUpperCase()}
                    </span>
                    <strong
                      style={{
                        fontFamily: "'Cabinet Grotesk',sans-serif",
                  ...
                  >
                    {card.guide?.dynamic ||
                      `${card.roleA} and ${card.roleB} see the world through different physics - that's where the friction comes from.`}
                  </p>
                  {card.guide && (
                    <p
                      style={{
                    ...
                    >
                      <strong>Do:</strong> {card.guide.do}{" "}
                      <strong>Don&apos;t:</strong> {card.guide.dont}
                    </p>
                  )}
                </div>
              ))}
        ...
        {missingRoles.map((role) => {
          ...
          return (
            ...
            >
              ...
              >
                {MISSING_ROLE_IMPACT[role]} Every handoff that role would
                normally own becomes a trust-tax on the rest of the team.
                Someone improvises. Someone waits. Something drops.
              </p>
              {candidate && (
                <p
                  style={{
              ...
              >
                <strong>If hiring externally:</strong> Look for someone whose
                track record matches the {role} pattern above - not just a title
                match, but the actual behavioral signature.
              </p>
            </div>
          );
        })}
                ...
                >
                  ...
                  >
                    <p style={{ margin: "0 0 10px" }}>
                      <strong>Superpower:</strong> {insight.superpower}
                    </p>
                    <p style={{ margin: "0 0 10px" }}>
                      <strong>Your leak:</strong> {insight.blindSpot}
                    </p>
                    <p style={{ margin: 0 }}>
                      <strong>This week:</strong> {insight.growthEdge}
                    </p>
                  </div>
                </details>
              );
        ...
        <div className="space-y-8">
          <div className="border-t-2 border-black pt-8">
            ...
            <p className="text-gray-500">
              {domain} - {teamMembers.length} members
            </p>
          </div>
        ...
        <div className="bg-black text-white rounded-2xl p-8 md:p-12 space-y-6">
          ...
          <div className="max-w-xl mx-auto bg-white/10 border border-white/20 rounded-xl p-4 space-y-3">
            <p className="text-xs text-gray-400 uppercase tracking-wider font-bold">
              Share this link with your tribe:
            </p>
            ...
            <p className="text-xs text-gray-500">
              Anyone with this link can take the assessment and automatically
              join the {domain} tribe map.
            </p>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-2">
            <Link
              href={`/assessment?domain=${encodeURIComponent(domain || "")}`}
            >
