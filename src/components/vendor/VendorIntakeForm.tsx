...
const STEP_FIELDS: string[][] = [
  ['legal_entity_name', 'dba_name', 'state_of_incorporation', 'year_founded', 'website', 'linkedin', 'hq_address', 'facility_address', 'num_employees', 'contact_name', 'contact_title', 'contact_email', 'contact_phone', 'principals'],
  ['facility_type', 'facility_classification', 'synthesis_method', 'monthly_capacity', 'peptide_products', 'purity_levels', 'sterile_fill', 'cold_chain', 'chain_of_custody', 'batch_traceability_practices'],
  ['qms', 'third_party_testing', 'testing_lab_name', 'independent_testing_willingness', 'coa_lot_specific', 'coa_publicly_viewable', 'coa_public_url', 'testing_protocols', 'identity_confirmation_method', 'stability_testing', 'recall_history', 'batch_docs'],
  ['moq', 'lead_time', 'pricing_model', 'payment_terms', 'distribution_channels', 'references', 'pricing_compound_1', 'pricing_unit_size_1', 'pricing_price_1', 'pricing_moq_tier_1', 'doc_price_list'],
  ['fda_registration', 'dea_registration', 'state_licenses', 'manufacturing_certifications', 'shipping_jurisdictions', 'product_labeling', 'buyer_eligibility', 'last_fda_inspection', 'fda_outcome', 'warning_letters', 'insurance'],
  ['doc_coa', 'doc_cgmp', 'doc_fda', 'doc_insurance', 'doc_sop', 'doc_additional'],
]
...
function FieldError({ formik, name }: { formik: FormikProps<FormValues>; name: string }) {
  ...
  return <p className="mt-1 text-[11px] font-medium" style={{ color: 'oklch(0.55 0.2 25)' }}>{String(message)}</p>
}

function Card({ icon: Icon, title, stepNumber, children }: {
    ...
    <Card icon={Building2} title="Company Information" stepNumber={1}>
      <div className="grid sm:grid-cols-2 gap-5">
        <div className={fw}>
          ...
          <input type="text" name="legal_entity_name" value={(v.legal_entity_name as string) ?? ''} onChange={formik.handleChange} placeholder="Registered business name" className={inp} />
          <FieldError formik={formik} name="legal_entity_name" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>DBA / Trade Name</label>
          <input type="text" name="dba_name" value={(v.dba_name as string) ?? ''} onChange={formik.handleChange} placeholder="Operating name, if different" className={inp} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>State / Country of Incorporation {req}</label>
          <input type="text" name="state_of_incorporation" value={(v.state_of_incorporation as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. Delaware, USA" className={inp} />
        ...
        <div className={fw}>
          ...
          <input type="number" name="year_founded" value={(v.year_founded as string) ?? ''} onChange={formik.handleChange} placeholder="YYYY" min="1900" max="2026" className={inp} />
          <FieldError formik={formik} name="year_founded" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Website</label>
          <input type="url" name="website" value={(v.website as string) ?? ''} onChange={formik.handleChange} placeholder="https://" className={inp} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Company LinkedIn</label>
          <input type="url" name="linkedin" value={(v.linkedin as string) ?? ''} onChange={formik.handleChange} placeholder="https://linkedin.com/company/..." className={inp} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Headquarters Address {req}</label>
          <input type="text" name="hq_address" value={(v.hq_address as string) ?? ''} onChange={formik.handleChange} placeholder="Full street address, city, state, zip" className={inp} />
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <input type="text" name="facility_address" value={(v.facility_address as string) ?? ''} onChange={formik.handleChange} placeholder="If different from HQ" className={inp} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Number of Employees {req}</label>
          <select name="num_employees" value={(v.num_employees as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          ...
          <input type="text" name="contact_name" value={(v.contact_name as string) ?? ''} onChange={formik.handleChange} placeholder="Full name" className={inp} />
          <FieldError formik={formik} name="contact_name" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Title / Role {req}</label>
          <input type="text" name="contact_title" value={(v.contact_title as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. VP of Sales" className={inp} />
          <FieldError formik={formik} name="contact_title" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Email {req}</label>
          <input type="email" name="contact_email" value={(v.contact_email as string) ?? ''} onChange={formik.handleChange} placeholder="name@company.com" className={inp} />
          <FieldError formik={formik} name="contact_email" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Phone</label>
          <input type="tel" name="contact_phone" value={(v.contact_phone as string) ?? ''} onChange={formik.handleChange} placeholder="+1 (555) 000-0000" className={inp} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Principals / Founders {req}</label>
          <textarea name="principals" value={(v.principals as string) ?? ''} onChange={formik.handleChange} placeholder="Name — Title — Ownership % (all individuals with >10% ownership)" className={ta} />
...
function StepManufacturing({ formik }: StepProps) {
  const v = formik.values
  return (
    <Card icon={FlaskConical} title="Manufacturing & Capabilities" stepNumber={2}>
      <div className="grid sm:grid-cols-2 gap-5">
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Facility Type {req}</label>
          <select name="facility_type" value={(v.facility_type as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          ...
          <select name="facility_classification" value={(v.facility_classification as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          ...
          <select name="synthesis_method" value={(v.synthesis_method as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          ...
          <input type="text" name="monthly_capacity" value={(v.monthly_capacity as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. 50,000 vials / month" className={inp} />
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <textarea name="peptide_products" value={(v.peptide_products as string) ?? ''} onChange={formik.handleChange} placeholder="List all compounds currently manufactured (BPC-157, TB-500, Semaglutide, Tirzepatide, etc.)" className={ta} />
        ...
        <div className={fw}>
          ...
          <input type="text" name="purity_levels" value={(v.purity_levels as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. ≥98% HPLC purity" className={inp} />
        ...
        <div className={fw}>
          ...
          <select name="sterile_fill" value={(v.sterile_fill as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <textarea name="cold_chain" value={(v.cold_chain as string) ?? ''} onChange={formik.handleChange} placeholder="Temperature-controlled storage, cold chain logistics, shipping capabilities" className={ta} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          ...
          <select name="chain_of_custody" value={(v.chain_of_custody as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
        ...
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Batch Traceability Practices</label>
          <textarea name="batch_traceability_practices" value={(v.batch_traceability_practices as string) ?? ''} onChange={formik.handleChange} placeholder="Existing traceability practices beyond QR/blockchain willingness above (e.g. internal lot logging, ERP tracking)" className={ta} />
        </div>
      </div>
    </Card>
  )
    ...
    <Card icon={ShieldCheck} title="Quality Assurance" stepNumber={3}>
      <div className="grid sm:grid-cols-2 gap-5">
        <div className={fw}>
          ...
          <select name="qms" value={(v.qms as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          ...
          <select name="third_party_testing" value={(v.third_party_testing as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Third-Party Testing Lab Name</label>
          <input type="text" name="testing_lab_name" value={(v.testing_lab_name as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. Janoshik Analytical, MZ Biolabs" className={inp} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          ...
          <select name="independent_testing_willingness" value={(v.independent_testing_willingness as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
            {['Yes, ongoing','Yes, one-time per new listing','No'].map(o=><option key={o}>{o}</option>)}
          </select>
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>COA Batch/Lot-Specific?</label>
          <select name="coa_lot_specific" value={(v.coa_lot_specific as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
            {['Yes','No'].map(o=><option key={o}>{o}</option>)}
          </select>
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>COA Publicly Viewable Before Purchase? {req}</label>
          <select name="coa_publicly_viewable" value={(v.coa_publicly_viewable as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
            {['Yes','No'].map(o=><option key={o}>{o}</option>)}
          </select>
        ...
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>COA Public Link (if applicable)</label>
          <input type="url" name="coa_public_url" value={(v.coa_public_url as string) ?? ''} onChange={formik.handleChange} placeholder="https://" className={inp} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Testing Protocols {req}</label>
          <textarea name="testing_protocols" value={(v.testing_protocols as string) ?? ''} onChange={formik.handleChange} placeholder="HPLC, Mass Spec, Endotoxin, Sterility, Microbial, Identity, Potency" className={ta} />
          <FieldError formik={formik} name="testing_protocols" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Identity Confirmation Method</label>
          <input type="text" name="identity_confirmation_method" value={(v.identity_confirmation_method as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. Mass spectrometry" className={inp} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Stability Testing Program {req}</label>
          <select name="stability_testing" value={(v.stability_testing as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
        ...
        <div className={fw}>
          ...
          <select name="recall_history" value={(v.recall_history as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <textarea name="batch_docs" value={(v.batch_docs as string) ?? ''} onChange={formik.handleChange} placeholder="Batch record system, COA generation process, document retention policy" className={ta} />
        </div>
      </div>
    </Card>
  )
    ...
    <Card icon={Briefcase} title="Commercial Terms" stepNumber={4}>
      <div className="grid sm:grid-cols-2 gap-5">
        <div className={fw}>
          ...
          <input type="text" name="moq" value={(v.moq as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. 1,000 vials" className={inp} />
          <FieldError formik={formik} name="moq" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Standard Lead Time {req}</label>
          <input type="text" name="lead_time" value={(v.lead_time as string) ?? ''} onChange={formik.handleChange} placeholder="e.g. 4–6 weeks from PO" className={inp} />
          <FieldError formik={formik} name="lead_time" />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Pricing Model {req}</label>
          <select name="pricing_model" value={(v.pricing_model as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={fw}>
          ...
          <select name="payment_terms" value={(v.payment_terms as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <textarea name="distribution_channels" value={(v.distribution_channels as string) ?? ''} onChange={formik.handleChange} placeholder="Current distribution partnerships, clinic networks, retail channels" className={ta} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>References {req}</label>
          <textarea name="references" value={(v.references as string) ?? ''} onChange={formik.handleChange} placeholder="2–3 current client references (company name, contact, relationship duration)" className={ta} />
        ...
        <div className="sm:col-span-2">
          ...
          {pricingRows === 1 && <FieldError formik={formik} name="pricing_compound_1" />}
          {pricingRows < 5 && (
            <button
              type="button"
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <input
            ...
            onChange={(e) => formik.setFieldValue('doc_price_list', e.target.files?.[0])}
          ...
          {v.doc_price_list instanceof File && (
            <p className="mt-1 text-[10px] font-medium" style={{ color: 'oklch(0.52 0.12 70)' }}>{v.doc_price_list.name}</p>
          )}
        </div>
      </div>
    </Card>
    ...
    <Card icon={Scale} title="Regulatory & Compliance" stepNumber={5}>
      <div className="grid sm:grid-cols-2 gap-5">
        <div className={fw}>
          ...
          <input type="text" name="fda_registration" value={(v.fda_registration as string) ?? ''} onChange={formik.handleChange} placeholder="FEI number" className={inp} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>DEA Registration {req}</label>
          <select name="dea_registration" value={(v.dea_registration as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <textarea name="state_licenses" value={(v.state_licenses as string) ?? ''} onChange={formik.handleChange} placeholder="State — License Type — Number (all active licenses)" className={ta} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Manufacturing Certifications</label>
          <textarea name="manufacturing_certifications" value={(v.manufacturing_certifications as string) ?? ''} onChange={formik.handleChange} placeholder="ISO, GMP, or other manufacturing certifications held (body, cert number, expiry)" className={ta} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Shipping Jurisdictions {req}</label>
          <input type="text" name="shipping_jurisdictions" value={(v.shipping_jurisdictions as string) ?? ''} onChange={formik.handleChange} placeholder="Countries/states you ship to and from" className={inp} />
        ...
        <div className={fw}>
          ...
          <select name="product_labeling" value={(v.product_labeling as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
        ...
        <div className={fw}>
          ...
          <select name="buyer_eligibility" value={(v.buyer_eligibility as string) ?? ''} onChange={formik.handleChange} className={sel}>
            <option value="">Select option</option>
        ...
        <div className={fw}>
          ...
          <input type="text" name="last_fda_inspection" value={(v.last_fda_inspection as string) ?? ''} onChange={formik.handleChange} placeholder="MM/YYYY or N/A" className={inp} />
        </div>
        <div className={fw}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>FDA Inspection Outcome</label>
          <select name="fda_outcome" value={(v.fda_outcome as string) ?? ''} onChange={formik.handleChange} className={sel}>
        ...
        <div className={`${fw} sm:col-span-2`}>
          ...
          <textarea name="warning_letters" value={(v.warning_letters as string) ?? ''} onChange={formik.handleChange} placeholder="FDA warning letters or enforcement actions in past 5 years. Enter 'None' if not applicable." className={ta} />
        </div>
        <div className={`${fw} sm:col-span-2`}>
          <label className={lbl} style={{ color: 'oklch(0.52 0.12 70)' }}>Insurance Coverage {req}</label>
          <textarea name="insurance" value={(v.insurance as string) ?? ''} onChange={formik.handleChange} placeholder="Product liability, general liability, professional liability coverage amounts" className={ta} />
  ...
  return (
    <Card icon={FolderOpen} title="Document Upload" stepNumber={6}>
        ...
        {DOCS.map((f) => (
          ...
          >
            ...
            {v[f.name] instanceof File && (
              <p className="text-[10px] font-medium" style={{ color: 'oklch(0.52 0.12 70)' }}>{(v[f.name] as File).name}</p>
            )}
      ...
      <p className="mt-3 text-[11px]" style={{ color: 'oklch(0.6 0.02 50)', fontFamily: 'var(--font-body)' }}>
        <span style={{ color: 'var(--gold)' }}>*</span> {REQUIRED_FIELD_COUNT} required fields across all sections
      </p>
    </Card>
  )
}
...
function StepBar({ active, onStepClick }: { active: number; onStepClick: (target: number) => void }) {
  return (
...
export default function VendorIntakeForm() {
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const res = await fetch('/api/vendor-intake', {
          ...
          body: JSON.stringify({ formData, files: fileEntries, sourceUrl: window.location.href }),
        })
  ...
  async function goToStep(target: number) {
    if (target > active) {
      ...
      if (stepErrorFields.length) {
        ...
        stepErrorFields.forEach((name) => { touched[name] = true })
  ...
  return (
    <form name="vendor-intake" onSubmit={formik.handleSubmit} style={{ fontFamily: 'var(--font-body)' }}>
