...
import {Search, X, Send, Sparkles, ArrowRight, Globe, BookOpen, FileText, RotateCcw} from 'lucide-react'
...
function parseInline(text: string, depth = 0): React.ReactNode {
  ...
  const linkStyle = {color: 'oklch(0.82 0.15 75)', textDecoration: 'underline', textUnderlineOffset: '2px'} as const
  const codeStyle = {background: 'rgba(255,255,255,0.1)', padding: '1px 5px', borderRadius: '3px', fontSize: '0.85em', fontFamily: 'monospace', color: 'oklch(0.85 0.12 75)'} as const
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    if (m[1] && m[2] !== undefined && m[3] !== undefined) {
      ...
      parts.push(
        <a key={k++} href={m[3]} target="_blank" rel="noopener noreferrer" style={linkStyle}>
          {parseInline(m[2], depth + 1)}
        </a>
      )
    } else if (m[4] !== undefined) {
      // **bold** — recurse so links inside bold still render
      parts.push(<strong key={k++} style={{color: 'rgba(255,255,255,0.95)', fontWeight: 600}}>{parseInline(m[4], depth + 1)}</strong>)
    } else if (m[5] !== undefined) {
      // *italic* — recurse
      parts.push(<em key={k++}>{parseInline(m[5], depth + 1)}</em>)
    } else if (m[6] !== undefined) {
      // `code`
      parts.push(<code key={k++} style={codeStyle}>{m[6]}</code>)
    } else {
      // bare https?:// URL
      parts.push(<a key={k++} href={m[0]} target="_blank" rel="noopener noreferrer" style={linkStyle}>{m[0]}</a>)
    }
...
function MessageContent({content}: {content: string}) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      blocks.push(
        <pre key={k++} style={{background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0.5rem', padding: '0.6rem 0.875rem', overflowX: 'auto', fontSize: '0.775rem', fontFamily: 'monospace', color: 'rgba(255,255,255,0.75)', margin: '0.2rem 0', lineHeight: 1.6}}>
          <code>{codeLines.join('\n')}</code>
        </pre>
      )
      i++; continue
    ...
    // Headings
    const h1m = line.match(/^# (.+)/);   if (h1m) { blocks.push(<p key={k++} style={{fontWeight: 700, fontSize: '0.95rem', color: 'white', margin: '0.15rem 0'}}>{parseInline(h1m[1])}</p>); i++; continue }
    const h2m = line.match(/^## (.+)/);  if (h2m) { blocks.push(<p key={k++} style={{fontWeight: 700, fontSize: '0.9rem', color: 'rgba(255,255,255,0.95)', margin: '0.15rem 0'}}>{parseInline(h2m[1])}</p>); i++; continue }
    const h3m = line.match(/^### (.+)/); if (h3m) { blocks.push(<p key={k++} style={{fontWeight: 600, fontSize: '0.85rem', color: 'oklch(0.82 0.15 75)', margin: '0.1rem 0'}}>{parseInline(h3m[1])}</p>); i++; continue }
    ...
    if (line.match(/^[\-\*•]\s+/)) {
      ...
      blocks.push(
        <ul key={k++} style={{margin: '0.1rem 0', padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.2rem'}}>
          {items.map((item, j) => (
            <li key={j} style={{display: 'flex', gap: '0.4rem', alignItems: 'flex-start'}}>
              <span style={{color: 'oklch(0.82 0.15 75)', flexShrink: 0, marginTop: '0.15rem', fontSize: '0.7rem'}}>▸</span>
    ...
    if (line.match(/^\d+\.\s+/)) {
      ...
      blocks.push(
        <ol key={k++} style={{margin: '0.1rem 0', padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.2rem'}}>
          {items.map((item, j) => (
            <li key={j} style={{display: 'flex', gap: '0.4rem', alignItems: 'flex-start'}}>
              <span style={{color: 'oklch(0.82 0.15 75)', flexShrink: 0, fontWeight: 600, minWidth: '1rem', fontSize: '0.75rem'}}>{j + 1}.</span>
              <span>{parseInline(item)}</span>
            </li>
          ))}
        </ol>
    ...
    if (line.match(/^-{3,}$/) || line.match(/^\*{3,}$/)) {
      blocks.push(<hr key={k++} style={{border: 'none', borderTop: '1px solid rgba(255,255,255,0.08)', margin: '0.3rem 0'}} />)
      i++; continue
    }

    // Blank line
    if (!line.trim()) { if (blocks.length > 0) blocks.push(<div key={k++} style={{height: '0.3rem'}} />); i++; continue }

    // Paragraph
    blocks.push(<p key={k++} style={{margin: 0, lineHeight: 1.65}}>{parseInline(line)}</p>)
    i++
  }

  return <div style={{display: 'flex', flexDirection: 'column', gap: '0.18rem'}}>{blocks}</div>
}
        ...
        <>
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{background: 'linear-gradient(135deg, oklch(0.82 0.15 75), oklch(0.6 0.22 50))'}}
                >
                ...
                <div>
                  ...
                  >25 years of enterprise intelligence</p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-4 sm:py-5 space-y-5">
              ...
              {chat.length === 0 && !showQuickNav && (
                <div className="py-1">
                  ...
                  >
                    Ask anything about RampRate — sourcing, Web3, case results, or enterprise strategy.
                  </p>
              ...
              {showQuickNav && (
                <div>
                  ...
                  <div className="space-y-1">
                    {quickResults.map(item => {
                      ...
                      return (
                        ...
                        >
                          <Icon size={14} className="shrink-0" style={{color: 'oklch(0.82 0.15 75)'}} />
                          <span className="flex-1 text-sm" style={{color: 'rgba(255,255,255,0.8)', fontFamily: 'var(--font-body)'}}>{item.title}</span>
                          <ArrowRight size={12} className="shrink-0" style={{color: 'rgba(255,255,255,0.2)'}} />
                        </button>
                      )
                    })}
                  </div>
                  <p
                    ...
                    style={{color: 'rgba(255,255,255,0.18)', borderTop: '1px solid rgba(255,255,255,0.05)', fontFamily: 'var(--font-mono)'}}
                  >Press Enter to ask AI instead</p>
                </div>
              )}
                ...
                <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: msg.role === 'user' ? '1rem 1rem 0.2rem 1rem' : '0.2rem 1rem 1rem 1rem',
            ...
            >
              ...
              >
                <Search size={15} className="shrink-0" style={{color: 'rgba(255,255,255,0.2)'}} />
                <input
                  ref={inputRef}
                  type="text"
                  value={query}
                ...
                <button
                  ...
                  onMouseEnter={e => {
                    if (!e.currentTarget.disabled) e.currentTarget.style.background = 'oklch(0.78 0.17 75)'
                  }}
              ...
              <div className="flex items-center justify-between mt-2 px-1">
                <span
                  ...
                  style={{color: 'rgba(255,255,255,0.13)', fontFamily: 'var(--font-mono)'}}
                >
                  <kbd className="px-1 rounded" style={{background: 'rgba(255,255,255,0.05)'}}>⌘K</kbd> open ·{' '}
                  <kbd className="px-1 rounded" style={{background: 'rgba(255,255,255,0.05)'}}>esc</kbd> close ·{' '}
                  <kbd className="px-1 rounded" style={{background: 'rgba(255,255,255,0.05)'}}>↵</kbd> send
                </span>
                <span
                  className="text-[10px] sm:hidden"
                  style={{color: 'rgba(255,255,255,0.13)', fontFamily: 'var(--font-mono)'}}
                >Tap send or press enter</span>
                <span
                  className="text-[10px]"
                  style={{color: 'rgba(255,255,255,0.1)', fontFamily: 'var(--font-mono)'}}
                >RampRate · $10B intel</span>
              </div>
            </div>
          </div>
        </>
