...
export default function FloatingCTA() {
  ...
  const handleJoinTeam = (e: React.FormEvent) => {
    ...
    if (domain.trim()) {
      router.push(
        `/flow/assessment?domain=${encodeURIComponent(domain.trim())}`,
      );
    }
  };

  return (
    <div className="flow-floating-cta-enter fixed bottom-0 left-0 right-0 z-50">
      {/* Urgency pulse line */}
      ...
      <div className="bg-black/95 backdrop-blur-xl border-t border-white/10 shadow-[0_-8px_40px_rgba(0,0,0,0.5)]">
        <div className="container mx-auto px-4 py-3">
          {!showTeamInput ? (
            <div
              key="main"
              className="flow-floating-cta-swap flex flex-col sm:flex-row items-center justify-between gap-3"
            >
              <div className="flex items-center gap-3 text-white">
                <Zap className="h-5 w-5 text-yellow-400 animate-pulse shrink-0" />
                <span className="text-sm sm:text-base font-bold">
                  <span className="text-yellow-400">
                    Don&#39;t guess your role.
                  </span>{" "}
            ...
            <div
              key="team-input"
              className="flow-floating-cta-swap flex flex-col sm:flex-row items-center gap-3"
            >
              <div className="flex items-center gap-2 text-white shrink-0">
                <Users className="h-5 w-5 text-blue-400" />
                <span className="text-sm font-bold">
                  Enter your company domain to join your team:
                </span>
              </div>
