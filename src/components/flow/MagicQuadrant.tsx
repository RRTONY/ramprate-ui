...
import {
  ...
  TooltipProvider,
  TooltipTrigger,
} from "@/components/flow/ui/tooltip";
...
const assessments = [
  {
    ...
    color: "bg-primary",
    description:
      "Operational Physics. The only tool that maps how energy moves between people to predict team velocity.",
  ...
  },
  {
    ...
    description:
      "Cognitive Preference. Great for self-awareness, but fails to predict performance. Use for parties, not payroll.",
    validity: "Debated",
  ...
  },
  {
    ...
    color: "bg-muted-foreground/30",
    description:
      "Behavioral Style. Good for communication style, but lacks depth in cognitive processing.",
  ...
  },
  {
    ...
    color: "bg-muted-foreground/30",
    description:
      "Conative Action. Measures striving instincts. Excellent for individual role fit, but isolated.",
  ...
  },
  {
    ...
    color: "bg-muted-foreground/30",
    description:
      "Core Motivations. Deeply spiritual and psychological, but too abstract for operational speed.",
  ...
  },
  {
    ...
    color: "bg-muted-foreground/30",
    description:
      "Soulprint. Cosmic alignment. Beautiful for purpose, irrelevant for P&L.",
  ...
  },
  {
    ...
    color: "bg-muted-foreground/30",
    description:
      "Team Roles. The closest ancestor, but relies on self-reporting rather than energetic physics.",
    validity: "Established",
    focus: "Team Roles",
...
export default function MagicQuadrant() {
  const [activePhase, setActivePhase] = useState<"static" | "kinetic">(
    "static",
  );
  const [selectedItem, setSelectedItem] = useState<
    (typeof assessments)[0] | null
  >(null);
  ...
  const filteredAssessments = assessments.filter((a) => {
    ...
    if (filterMode === "self")
      return (
        a.focus.includes("Individual") ||
        a.focus.includes("Motivation") ||
        a.focus.includes("Identity")
      );
    if (filterMode === "team")
      return (
        a.focus.includes("Team") ||
        a.focus.includes("Behavior") ||
        a.focus.includes("Action")
      );
    return true;
  });
    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 pt-12 md:pt-0 relative">
      ...
      <div className="text-center space-y-4 max-w-4xl mx-auto px-4">
        ...
        <p className="text-xl text-black/70 max-w-2xl mx-auto">
          Stop using{" "}
          <span className="font-bold text-black">personality tests</span> for{" "}
          <span className="font-bold text-primary">operational problems</span>.
        </p>
      </div>
      ...
      <div className="bg-white/90 backdrop-blur-md border border-black/5 rounded-2xl p-4 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4 mx-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm font-bold text-muted-foreground mr-2 hidden md:inline-block">
            Filter:
          </span>
      ...
      <div className="px-4">
        ...
        <div className="relative w-full aspect-square md:aspect-[16/9] max-h-[500px] bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/60 overflow-hidden shadow-2xl group mx-auto">
          ...
          <div
            className={`absolute inset-0 transition-opacity duration-1000 ${activePhase === "kinetic" ? "opacity-100" : "opacity-50"}`}
          >
          ...
          <div className="absolute inset-0 pointer-events-none">
            ...
            <div className="absolute top-3 left-3 text-[10px] md:text-xs font-black tracking-widest text-black/50 uppercase bg-white/60 px-2 py-1 rounded backdrop-blur-sm">
              Internal / Psychological
            </div>
            <div className="absolute top-3 right-3 text-[10px] md:text-xs font-black tracking-widest text-black/50 uppercase bg-white/60 px-2 py-1 rounded backdrop-blur-sm">
              External / Operational
            </div>

            {/* Y-axis: Top = Self Discovery, Bottom = Team Execution */}
            <div className="absolute top-3 left-1/2 -translate-x-1/2 text-[10px] md:text-xs font-black tracking-widest text-black/50 uppercase bg-white/60 px-2 py-1 rounded backdrop-blur-sm">
              Self Discovery
            </div>
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 text-[10px] md:text-xs font-black tracking-widest text-black/50 uppercase bg-white/60 px-2 py-1 rounded backdrop-blur-sm">
              Team Execution
            </div>
      ...
      <div className="md:hidden px-4 space-y-4">
        ...
        {filteredAssessments.map((item) => (
          <Card
            key={item.name}
            className="p-4 flex items-center justify-between bg-white/80 backdrop-blur-sm border-black/5"
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${item.name === "Flow Circuit" ? "bg-primary text-white" : "bg-gray-100 text-black"}`}
              >
                {item.name[0]}
              </div>
              <div>
          ...
          >
            <Card className="p-6 shadow-2xl border-primary/20 bg-white/95 backdrop-blur-xl relative overflow-hidden">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold shadow-inner ${selectedItem.name === "Flow Circuit" ? "bg-primary text-white" : "bg-gray-100 text-black"}`}
                  >
                    {selectedItem.name[0]}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="block text-xs text-muted-foreground uppercase tracking-wider font-bold mb-1">
                      Research Standing
                    </span>
                    <span
                      className={`font-mono font-bold ${selectedItem.validity.includes("Pending") ? "text-gray-500" : selectedItem.validity.includes("High") ? "text-green-600" : "text-yellow-600"}`}
                    >
                  ...
                  <div>
                    ...
                    <span className="font-medium">
                      {selectedItem.name === "Flow Circuit"
                        ? "Team Velocity"
                        : "Self Awareness"}
                    </span>
                  </div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  Research Standing is our general, qualitative characterization
                  of each tool&#39;s published literature - not a precise
                  validity coefficient. See{" "}
                  <span className="font-medium">/flow/science</span> for The
                  Flow Circuit&#39;s own current validation status.
                </p>
                ...
                <Button
                  className="w-full font-bold text-md h-12 rounded-xl shadow-lg hover:shadow-xl transition-all"
                  asChild
                >
                  <a
