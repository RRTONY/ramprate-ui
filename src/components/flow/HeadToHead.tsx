...
export default function HeadToHead({ members }: HeadToHeadProps) {
  ...
  const analyzeDynamics = (roleA: Role, roleB: Role) => {
    ...
    if (roleA === roleB) {
      return {
        ...
        description:
          "High alignment. You speak the same language, but may share the same blind spots.",
    ...
    switch (pairs) {
      case "Amplifier-Spark":
        return {
          ...
          description:
            "Explosive creativity. Spark ignites, Amplifier expands. Risk of burnout or lack of grounding.",
      ...
      case "Filter-Spark":
        return {
          ...
          color: "text-red-400",
          description:
            "Classic tension. Spark wants to go, Filter wants to pause. This friction is healthy if managed, toxic if personal.",
      ...
      case "Ground-Spark":
        return {
          ...
          color: "text-blue-400",
          description:
            "Spark flies high, Ground stays low. Essential for turning dreams into reality, but communication can be difficult.",
      ...
      default:
        return {
          ...
          description:
            "Different energies that can support each other if roles are clear.",
  ...
  const analysis =
    selectedA && selectedB
      ? analyzeDynamics(selectedA.role, selectedB.role)
      : null;
