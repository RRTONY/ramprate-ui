...
export default function SlackConnect({ teamId }: SlackConnectProps) {
  ...
  const teamDetailQuery = trpc.team.getById.useQuery(
    { id: teamId! },
    { enabled: !!teamId },
  );
  const teamDetail = teamDetailQuery.data;
  const isConnected = Boolean(teamDetail?.slackWebhookUrl);

  useEffect(() => {
    if (!teamDetail?.slackWebhookUrl) return;
    const frame = window.requestAnimationFrame(() => {
      setWebhookUrl((currentUrl) => currentUrl || teamDetail.slackWebhookUrl);
    });
        ...
        {!isConnected ? (
          <>
            <p className="text-sm text-gray-400">
              Get real-time pings in your team channel whenever a new member
