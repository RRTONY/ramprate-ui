...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| Managed content database           | `DATABASE_URL` is used server-side by the content data layer. Public routes read normalized posts, categories, post-category mappings, pages, media assets, page SEO, site settings, navigation, and supporting-content tables; image URLs remain references, not table-stored file bytes. | Use the managed runtime value only. Never commit a database URL or expose it to browser code.                                |
| Content updates                    | Managed content updates write directly to the normalized database through the restricted Flow admin console. The former generic `content_documents` table remains a rollback archive until relational parity is fully accepted.                                                            | Configure `ADMIN_EMAILS` with the allowed Flow account email addresses. No external CMS token or webhook secret is required. |
| Public form submissions            | Contact, newsletter, client, champions, engagement, payment, and supplier-intake payloads are saved as private `form_submissions` records. Established intake routes preserve their existing external forwarding.                                                                          | Set `GOOGLE_APPS_SCRIPT_URL` only when the existing Google Sheet forwarding is required; never expose it in browser code.    |
